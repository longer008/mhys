...
【程序计算结果｜事实，不得重算或替换】
以下内容均为程序已经计算出的事实。你只能解释这些结果，不得重新起卦、不得改动体卦/用卦/五行/体用关系/基础吉凶/起卦时间/动爻，也不得用自己的算法替换。
...
体用关系与基础吉凶：${wuxingRelation ? `${wuxingRelation.label}，${wuxingRelation.judgment}（${wuxingRelation.description}）` : "旧记录未保存。若体卦、用卦、五行仍已给出，可只按已给事实谨慎说明；不得自行重算体用关系或替换既有体用。"}

【解读规则】
1. 程序计算结果优先。你只解释，不重算；不能推翻或替换上述体卦、用卦、五行、体用关系、基础吉凶、起卦时间、动爻。
